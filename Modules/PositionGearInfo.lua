...
function AddOn:SetUpgradeTrackPositionBySlot(slot)
    ...
    local itemLevelShown = self.db.profile.itemLevel.show and not self.db.profile.itemLevel.onItem and slot.PGVItemLevel and slot.PGVItemLevel:IsShown()
    local itemLevelShownOnItem = self.db.profile.itemLevel.show and self.db.profile.itemLevel.onItem and slot.PGVItemLevel and slot.PGVItemLevel:IsShown()
...
function AddOn:SetInspectUpgradeTrackPositionBySlot(slot)
    local IsLeftSide = self:GetSlotIsLeftSide(slot, true)
    slot.PGVUpgradeTrack:ClearAllPoints()
    local itemLevelShown = self.db.profile.inspect.showILvl and not self.db.profile.itemLevel.onItem and slot.PGVItemLevel and slot.PGVItemLevel:IsShown()
    local itemLevelShownOnItem = self.db.profile.inspect.showILvl and self.db.profile.itemLevel.onItem and slot.PGVItemLevel and slot.PGVItemLevel:IsShown()
...
function AddOn:SetInspectGemsPositionBySlot(slot)
    ...
    local itemLevelShown = self.db.profile.inspect.showILvl and not self.db.profile.itemLevel.onItem and slot.PGVItemLevel and slot.PGVItemLevel:IsShown()
    local itemLevelShownOnItem = self.db.profile.inspect.showILvl and self.db.profile.itemLevel.onItem and slot.PGVItemLevel and slot.PGVItemLevel:IsShown()
    local upgradeTrackShown = self.db.profile.inspect.showUpgradeTrack and slot.PGVUpgradeTrack and slot.PGVUpgradeTrack:IsShown() and not PlayerGetTimerunningSeasonID()
    local isMainHand = slot == _G["InspectMainHandSlot"]
...
function AddOn:SetEnchantPositionBySlot(slot)
    ...
    if self.db.profile.enchants.collapse and slot.IsLeftSide == nil and upgradeTrackShown then
    ...
    elseif self.db.profile.enchants.collapse and slot.IsLeftSide == nil then
    ...
    elseif (not self.db.profile.itemLevel.show or itemLevelShownOnItem) and gemsShown and slot.IsLeftSide ~= nil and isSocketableSlot and isEnchantableSlot then
...
function AddOn:SetInspectEnchantPositionBySlot(slot)
    ...
    local itemLevelShown = self.db.profile.inspect.showILvl and not self.db.profile.itemLevel.onItem and slot.PGVItemLevel and slot.PGVItemLevel:IsShown()
    local itemLevelShownOnItem = self.db.profile.inspect.showILvl and self.db.profile.itemLevel.onItem and slot.PGVItemLevel and slot.PGVItemLevel:IsShown()
    local upgradeTrackShown = self.db.profile.inspect.showUpgradeTrack and slot.PGVUpgradeTrack and slot.PGVUpgradeTrack:IsShown()
    local gemsShown = self.db.profile.inspect.showGems and slot.PGVGems and slot.PGVGems:IsShown() and not PlayerGetTimerunningSeasonID()
    ...
    elseif (not self.db.profile.inspect.showILvl or itemLevelShownOnItem) and gemsShown and IsLeftSide ~= nil and isSocketableSlot and isEnchantableSlot then
        -- Adjust positioning for slots that have both gems and enchants visible
        slot.PGVGems:ClearAllPoints()
